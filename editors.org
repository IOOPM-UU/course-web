...
#+begin_attention
Most of this text is being written from scratch in 2018. For now, refer to the bootstrap sections on the [[./assignments/c-bootstrap.org][C bootstap labs]].
#+end_attention
...
Vissa tillägg av Tobias Wrigstad (huvudlärare på IOOPM 2011--2018) (speciella kortkommandon för IOOPM).
...
Ett annat sätt att lösa problemet är att använda [[key:⌘]] istället. Det betyder att Emacs ibland slåss med macOS om vad som händer när du trycker på kombinationer med den tangenten (macOS vinner typiskt), men över lag fungerar det och du kan i alla fall skriva brackets. I sådana fall behöver du klistra in följande rader i din =init.el=:
...
Det grundläggande problemet är förstås att de inte gör tangentbord med [[https://en.wikipedia.org/wiki/Space-cadet_keyboard][så många knappar som de hade förr]].
