...
2. Delete a hash table and free its used memory
...
Here, we can draw on our knowledge of the interal representation
in our choice of lookup uperations. If we know that the number of
bucket lists is 17, then we can easily carve up a test where we are sure to hit all the buckets. 
...
- Refactor the magic number 17 above
