...
├── list_linked.c         # linked list and iterator code 
├── list_linked.h         # linked list declarations
├── list_linked_tests.c   # linked list tests
...
convention for all /public/ types on this course, *meaning, you
will use this naming scheme for things in your code!* The
~entry_t~ type is only used internally by the hash table, so it
will not get a public name.
...
fprogram like so: =gcc -pedantic -Wall -g gdb-test.c= ([[../compiling.org][read about
...
we see a key $k_2$ such that $k_2 < k_1$.
...
entry /e/ such that /e/ = ($k_1$, $v_1$) such that $k_1 > key$, then
...
Furthermore, its previous entry of the entry is the sentinel (with
