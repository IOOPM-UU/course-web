...
- Create multiple opportunities to [[../about.org::*Redovisning%20och%20AU-Portalen][demonstrate mastery of
  achievements.]]
...
Note that [[./assignment2.org][Assignment 2]] will be built on-top of the data structures
...
- Have written about 1000 lines of code (LOC) plus another 1000
  ...
  probably need some help. Data point: Students on IOOPM who
  finished early wrote 1700--2500 LOC.)

** Prerequisites to Start

1. You should have finished the C boot-strapping exercises, which involves:
   1. Having familiarised yourself with the [[../simple.org][SIMPLE programming methodology]], and understanding key concepts like /dodging/ and /cheating/.
   2. Having familiarised yourself with Emacs, [[../editors.org][the editor we are using for at least the C part of the course]].
2. You should have found a [[../pair-programming.org][partner to work with from your team]].
3. You should have [[../github.org][gotten a GitHub repo *from the course*]].
   Naturally, you are to place all files belonging to this
   assignment in the folder for Assignment 1.
4. Make sure you have [[../tools.org][a working development environment for C]].
...
you are a seasoned C programmer paired with a novice programmer,
...
#+begin_attention
Also, please -- *unless explicitly asked to* -- do not Google how
to write /X/ in C until you have read and tinkered with the
explanations of this document. In general, please refrain from
using Google and Stackoverflow (etc.) during this assignment --
use Piazza instead.
#+end_attention

The entire assignment is laid out as a series of steps with the
intended velocity of 1-2 steps per weekday *on average*. Note that
not all steps have equal length. Expect early steps to take longer
(inverse proportional to your past coding experience perhaps?). If
you want to plan ahead, *read ahead* and take notes of what is
coming up. Early on, expect to be stuck on a compile error on a
single line for an hour at times (ask for help if stuck on a
compile error >20 minutes). Later on, expect to be mostly stuck
debugging some behaviour in your program.
/Bottom-line: how long each step will take is hard to plan, and is
going to be very individual./

** Organisation of Files
If you are relatively new to programming, it may not be clear to
you what the file structure could look like for this assignment.
Here is an example of what your directory /could/ look like when you
are done with the entire assignment.
...
with ~#pragma once~[fn::All lines starting with ~#~ are
...
Note that the program that you are going to read is quite awful
for many reasons:

1. The number of entries in the map is static -- chosen at
   compile-time[fn::This means that we must know what the user is
   going to do before the program runs, and that a user's
   interaction must not vary too much, or we will have to change
   the size in the source code and recompile the program.]).
...
tables work. This is a good opportunity to use your "search fu"[fn::As in /search skills/, cf. "kung fu".].
...
- Insertion and lookup are fast, and $O(1)$[fn::This is usually
  pronounced "constant time".]
...
The lack of support for deletion is actually rooted in a bigger
...
representation (meaning how the abstract notion of a hash table
mapping keys to values is stored in memory) so that:

1. Each index in the array covers a dynamic number of keys[fn::By
   dynamic, we mean a value which is decided at run-time, and is
   not fixed.] instead of a single key; and
2. Each value in the array is a sequence of (key,value)
   pairs so that each entry in the map is represented by a
   corresponding (key,value) pair (that we will call /an entry/).
...
- Insertion and lookup are still fast -- the array let's us jump
  ...
  the bucket (typically $O(n)$[fn::This is normally pronounced
  "linear time."], where $n$ is the number of entries in the
  bucket -- typically a quite small number)
...
Development of hash functions is super interesting, quite
subtle and out of scope for this course.

For concreteness, here is a very basic hash function for hashing
a string which produces an integer from a string by calculating
the sum of all ASCII values of all the characters.
#+BEGIN_SRC c
...
#+END_SRC
(Later in this document, we will see at least one better hash
...
*For now, we will [[../simple.org::*Dodging:%20Not%20Quite%20Cheating][dodge]] and consider only lists that store
integers.* This seems like a good simplification, and innocuous
too -- we know how to undo it later by adding a hash function.
...
size.]. For the rest of this assignment, we will stick with a
linked structure.

Following the [[../simple.org][SIMPLE methodology]], we are going to /dodge/ and
...
#+begin_attention
Always make a note of [[../simple.org::dodge][dodges]] and [[../simple.org::cheat][cheats]] so that you can revisit
...
#+NAME: fig_ht_2
#+CAPTION: Hash table with keys 4 $\mapsto$ bar, 17 $\mapsto$ foo, and 208 $\mapsto$ "baz".
[[../images/ht2.png]]

# This code can be used to generate uglier forms of the hash tables if keynote "sources" are ever lost
...
*** Placement of ~struct hash_table~ and ~struct entry~ etc.

The placement of structs is important. We want data structures to
be opaque, meaning it should not be possible for a client to know
how they are implemented. This preserves maximal freedom for us to
change the implementation as we continue to improve our code, and
also helps us protect the invariants of the data structure.
...
Place the structs in the right places, and know your motivation
for what you chose.

#+begin_tip
This is a good time to start thinking about [[../achievements.org::A3][Achievement A3]].
#+end_tip
...
definitely use that to demonstrate [[../achievements.org::R52][Achievement R52]].
#+end_tip
...
program like so: =gcc -pedantic -Wall -g gdb-test.c= ([[../compiling.org][read about
...
backtrace[fn::A backtrace is a list of all the functions that were
called (but not returned) on the way to the current point of the
program.] in the event of a segfault.
#+end_tip
...
Start by writing a [[../glossary.org][stub]] for the functions under test so we can
...
to you which may change the [[../glossary.org][signature]].
...
#+begin_important
Before we begin, [[../cunit.org][read the page on the CUnit test framework]] which
...
that it fails. This is in line with the [[../simple.org][SIMPLE]] approach (always
...
Start by writing a [[../glossary.org][stub]] for the function under test so we can
compile and run a (failing) test:
#+BEGIN_SRC c
  char *ioopm_hash_table_lookup(ioopm_hash_table_t *ht, int key)
#+END_SRC
Note that there are some design decisions left to you which may
change the [[../glossary.org][signature]].
...
[[../glossary.org][function prototypes]] for private functions topside of the =.c=
...
1. Make sure that your code compiles with the [[../compiling.org][appropriate flags]]
   turned on. Preferably using a [[../makefiles.org][Makefile]]. Once your makefile
   has grown a bit (and there are more files and more tests),
   you should use it to demonstrate [[../achievements.org::U57][Achievement U57]].
2. [[../github.org][Check your code into the approprate GitHub repos]]. Tag your
...
#+begin_tip
This is a good time to start /thinking/ about [[../achievements.org::A1][Achievement A1]].
#+end_tip
...
losing the $O(1)$ insertion to speed up search be an optimisation
...
#+CAPTION: Newly initialised hash with inlined dummy entries with 4 $\mapsto$ bar, 17 $\mapsto$ foo and 208 $\mapsto$ baz. The entry with key 4 is the /previous entry/ of the entry with key 208.
#+NAME: fig_ht_4
[[../images/ht4.png]]
...
is almost always worth it, so long as the costs are moderate. If
we have a large number of buckets, and a small number of entries
in each bucket, the ratio of dummies per entry will be high. As
long as the number of buckets is fixed to 17, the overhead is 17
entries, which is negligible on a modern machine.
...
4. Make sure that your code compiles with the [[../compiling.org][appropriate flags]]
   turned on. Preferably using a [[../makefiles.org][Makefile]].
5. Make sure that the code passes your tests.
6. [[../github.org][Check your code into the appropriate GitHub repos]]. Tag your
...
1. Let the function return /two/ values, adding one boolean value
   ...
   only alloed if success is ~true~).
2. Add a separate public function for checking whether a key is
   ...
   the program using e.g., an [[../glossary.org][assertion]] if an invalid key is
   detected.
...
#+begin_tip
This aspect of the assignment is a good fit for [[../achievements.org::I24][Achievement I24]].
#+end_tip
...
character ([[../pointers.org][if your mind boggles at the thought of a pointer to
another pointer, you can look here]]).

This design now very neatly supports both looking up and replacing
an existing entry:
...
- This seems to work well for lookup, but not for remove since the
  remove function frees the entry that holds the value whose
  location we would return (read that sentence again until you get
  it). In other words, by definition, remove cannot return a
  pointer to a place in the hash table, because it needs to remove
  that very place. One possibility is to change remove to simply
  not return the value removed, but this may force situations
  where extra lookups must be inserted[fn::On the other hand,
  lookups are typically a lot more frequent than removes.]. (It is
  also possible to hang on to the value even after the entry is
  free'd, e.g. by storing it in the dummy, so that it is possible
  to return it's address.)
- Note that keeping a pointer to a value in a hash table can be
  dangerous if something causes the value to move while we are
  keeping this pointer around. This needs to be reflected in the
  documentation.
...
1. Handle failing lookups by one of the alternatives above
   Note that your choice might force you to change
   the [[../glossary.org][signature]] of the function (changing the return type
   and/or adding an additional parameter). Make sure to
   properly document your choice in the comments in the
   header-file.
...
3. Make sure that your code compiles with the [[../compiling.org][appropriate flags]]
   turned on. Preferably using a [[../makefiles.org][Makefile]].
4. Make sure that the code passes your tests.
5. [[../github.org][Check your code into the appropriate GitHub repos]]. Tag your
...
Furthermore, the [[../glossary.org][signature]] and comments for
...
or use a single free to free more than one block. (See [[../misc/l%C3%A4nkade%20strukturer,%20notation%20och%20free.pdf][this slide
...
Although out of scope for this course, use-after-free bugs are
classic security exploits of C programs.
...
2. *Put allocation and deallocation in separate functions.* We
   ...
   ~entry_destroy()~ (even if the latter only contains a call to ~free()~).
...
2. Implement ~entry_destroy()~ to free a single entry. Use it in
   remove to free the memory for a successfully removed entry.
...
5. Make sure that your code compiles with the [[../compiling.org][appropriate flags]]
   turned on. Preferably using a [[../makefiles.org][Makefile]].
6. Make sure that the code passes your tests.
7. [[../github.org][Check your code into the appropriate GitHub repos]]. Tag your
...
the last step needed to rightfully claim that we have implemented a
...
*explicitly documented choice* that the hash table takes
*ownership* over all values it stores, it seems unreasonable to
assume that the life time of a value is tied to the life time of
the hash table. For example, imagine a value that is mapped to
different keys in two different tables. That seems like a
reasonable thing, but if we cannot destroy one hash table without
destroying all its values, the life times of the two hash tables
are now implicitly tied to each other. This seems brittle and easy
to get wrong. (Not to mention increasingly hairy if we want to use
the value in additional hash tables or other structures.)
...
your program by adding a free(x) on line 19". However, it will be
...
4. Run your tests through Valgrind. There are no reasons to see
   *any warnings or errors* and in addition, Valgrind should print
   out "No leaks possible" at the end[fn::If you run =valgrind= on
   ...
   [[../tools.org::Vagrant][Vagrant]].]. If you are using a Makefile, add an extra entry that
   allows you to run your tests through Valgrind, e.g., =make
   memtest=.
...
done with Ticket #1 -- [[../glossary.org][refactoring]]. For the time being, we will
...
4. [[../github.org][Check your code into the appropriate GitHub repos]]. Tag your
...
[[../glossary.org][function prototypes]] for above.
...
- (Step 6) ~ioopm_hash_table_size()~ for getting the size of hash table,
  ~ioopm_hash_table_empty()~ for checking if the hash table is empty, and
  ~ioopm_hash_table_clear()~ for clearing all entries in a hash table
  without removing the table.
- (Step 7) ~ioopm_hash_table_keys()~, and
  ~ioopm_hash_table_values()~ for getting arrays of all keys and
  values in the hash table.
- (Step 8) ~ioopm_hash_table_has_key()~, and
  ~ioopm_hash_table_has_value()~ which uses the functions in Step
  7 for checking if a hash table contains a specific key or value.
- (Step 9) ~ioopm_hash_table_all()~ and ~ioopm_hash_table_any()~ --
  two utility functions for checking if all (any) entries (entry)
  satisfies a given predicate, and ~ioopm_hash_table_apply_to_all()~ --
  a utility function for in-place bulk operation on entries.


#+CAPTION: Excerpt from =hash_table.h= -- functions to be implemented.
#+BEGIN_SRC c
...
1. Implement ~ioopm_hash_table_size()~,
   ...
   optimisations in your [[../glossary.org][backlog]].
...
straightforward, and not that different from previous functions.
...
#+begin_tip
This could be used as part of demonstrating [[../achievements.org::H19][Achievement H19]].
#+end_tip
...
and ~void *~) and returning a ~bool~[fn::Hint -- make a typedef for this so the type becomes readable!]. We will return to the ~x~
...
*** Function Pointers (General Information, and a Good ~typedef~ for ~ioopm_apply_function~)
...
Note the signature of this function -- it takes an integer, a string and a "something" -- and returns a boolean.
Let's make a typedef for this:
...
1. Implement ~ioopm_hash_table_all()~, ~ioopm_hash_table_any()~, and
   ~ioopm_hash_table_apply_to_all()~
   including giving a type for ~ioopm_apply_function~.
...
3. The tests for ~ioopm_hash_table_has_key()~ and
   ...
   Also write a test for ~ioopm_hash_table_apply_to_all()~.
4. Make sure that your code compiles with the [[../compiling.org][appropriate flags]]
   turned on. Preferably using a [[../makefiles.org][Makefile]].
...
So that's what we are going to do next: a /linked list/, and an /iterator/.

The listing below lists the functions you should implement for the
linked list. They are reminiscent of the functions in the hash
table. Stick with the same approach (test-driven, classical) as
before. The kinds of reasoning that we did for the hash table
works here too, e.g., there are internal functions that can serve
as the building blocks of public functions, and more specific
public functions can (sometimes) delegate to more general public
functions.
...
- How should failure be handled (e.g., inserting at an invalid
  index)? (You may alter signatures to deal with failure in the
  same way we did for the hash table.) The [[./lists.org][linked list overview]]
...
*NOTE* that ~ioopm_char_predicate~ and ~ioopm_apply_char_function~ must be defined
to work just like ~ioopm_predicate~ and ~ioopm_apply_function~, but with ~char~
replaced with ~int~. [[step13][Later]], we will be able to merge these two functions. 
...
- Start with basic functions like create, destroy, append, and
  prepend. Then move on to remove etc. Baby steps!
...
- Format your code nicely. You could even use a tool like =astyle=
  or =indent=[fn::Use e.g. =indent -nut -gnu -npsl -i2 file.c=.],
  or, in Emacs' =C-x h= to mark the entire buffer followed by =M-x
  indent-region=.


*Note:* There are *optional*, generic [[./lists.org][linked list instructions]]
available in a separate document, which was not written
specifically for this assignment. Thus, they are not divided into
steps etc. In 2018, many students were confused by this as they
started following that document, which had a separate structure.
The "Finish This Step" summary is below in /this/ document. You
may want to read about linked lists first and then go here and
implement the linked list -- or whatever works for you.

In that spirit, although the instructions in the link above
mention pointers to pointers, sticking with the dummy node is a
fine choice. For now, we will [[../simple.org::*Dodging:%20Not%20Quite%20Cheating][dodge]] and consider only lists that
store integers. (Feel free to tackle storing generic elements now
if you feel up to it, but if you are new to C, better to wait
until [[step13][Step 13]].) We will undo this simplification later, but it a
good initial simplification that makes things easy to test.


*** Finish This Step

1. Implement /all/ the functions listed in the header file above.
...
elements. Iterators can make code simple. For example, here is
a listing that prints all the integers in a linked list:
...
A generic introduction to iterators that builds on the
generic linked list page can be found [[./iterators.org][here]].

In this step, we shall extend the list with the ability to create
an iterator. Thus, such a function must be added to the public
list interface:
...
*** Use the ~size_t~ Data Type Instead of ~int~ in the Right Places
...
at least the $[−32,768, +32,767]$ range; thus, it is at least 16
bits in size.
#+END_QUOTE
...
#+begin_tip
~const~ qualifiers could be used to demonstrate [[../achievements.org::G15][Achievement G15]].
#+end_tip


*** Finish This Step
1. Make the required (and, optionally, optional) changes above:
   refactor to use ~size_t~ and return a list of keys instead of
   an array (and optionally add ~const~ qualifiers).
...
** Step 13: Supporting Arbitrary Data as Elements, Keys and Values <<step13>>
#+begin_tip
[[../achievements.org::E10][Achievement E10]] is related to this step.
#+end_tip

As you were looking at [[./lists.org][the text on linked lists]], you may have
...
arbitrary data. We will follow the trick in [[./lists.org::*Adding%20Genericity][the text on linked
...
a definition of ~elem_t~ you can use [[./lists.org::elem_t][here]].

Function pointers are going to be stored inside the list and hash
table structs and be passed in as arguments to the constructors,
[[Pass Function Pointers to the List's Constructor][which will be discussed shortly]].

Since C does not have run-time type information, we
...
#+begin_tip
Changing the signatures of functions can be frustrating. Assume
for example that you have lots of lines of code passing in
integers to hash table functions. These will no longer work as
keys and values expect ~elem_t~:s from now on. As an example, we
can use ~ioopm_hash_table_insert(ht, 42, "4711")~. 

An easy way to fix this is to introduce macros for creating
elements, e.g.:

#+BEGIN_SRC c
#define int_elem(i) (elem_t) { .i=(i) }
#define str_elem(s) (elem_t) { .s=(i) }
#+END_SRC

which allows us to rewrite the above as ~ioopm_hash_table_insert(ht, int_elem(42), str_elem("4711"))~. 
Another option is to define a macro for the entire function:

#+BEGIN_SRC c
#define ioopm_int_str_ht_insert(ht, i, s) \
   ioopm_hash_table_insert(ht, int_elem(i), str_elem(s))
#+END_SRC

Now we can simply rename all calls to ~ioopm_hash_table_insert~ to
~ioopm_int_str_ht_insert~.
#+end_tip
...
Soon we will be adding a hash function type. Then you will have to
think in similar ways: where does it's function type belong?

Note: you don't want separate ~elem_t~'s for separate data
structures. One for the entire program is enough, and declared in
one place.
...
code again. Some example hash functions are [[hash_function][further down in this
...
*An bonus of this trick* is that your existing tests should more
of less keep working as-is, since they are all written with
integers as hash table keys.

#+CAPTION: A function that extracts a given int key from an ~elem_t~ (we could improve this by hashing on the int key, but that's out of scope for this course!)
#+BEGIN_SRC c
...
1. Change the type of elements, keys and values to ~elem_t~ in the
   signature of functions (including constructors) and in structs.
2. Create =common.h= and populate it with suitable type
   definitions and declarations (including a typedef for the hash function).
...
6. Make sure that your code compiles with the [[../compiling.org][appropriate flags]]
   turned on. Preferably using a [[../makefiles.org][Makefile]].
7. Run your tests through Valgrind. Preferably using a [[../makefiles.org][Makefile]].
8. Make sure that the code passes your tests.
9. [[../github.org][Check your code into the appropriate GitHub repos]]. Tag your
...
*If you are feeling stressed to finish, you can [[../misc/freq-count.c][grab a copy of
...
#+begin_tip
This step is closely related to [[../achievements.org::O42][Achievement O42]] and [[../achievements.org::O43][Achievement O43]] (at least).
#+end_tip
...
cooking those up, you can get such files [[../misc/freq-count.zip][here]].

Run your program on each input file and inspect the result. Answer
the following questions.

- For each input, what are the top 3 functions?
- For each input, what are the top 3 functions *in your code*[fn::Meaning, "in your program" -- that you have written.], or is it library functions?
...
the next size to grow. The below figure provides such a table (for
small sizes) that you can use /if you want/.

#+CAPTION: Exponential growth in prime steps from $\approx 2^4$ upto $\approx 2^{14}$
#+BEGIN_SRC c
...
Note that with a design like this, that makes each bucket only
hold a few entries, the overhead of having one dummy node per
bucket gets pretty big.
...
Fixing this is a great opportunity to do the [[../achievements.org::O44][memory optimisation
achievement]].
#+end_note


*** Hash Function for Strings and Other Objects <<hash_function>>

As we have discussed previously -- a hash function gives a stable
interpretation of a datum as an integer. There is no single best-fit,
...
place to explore this concept further than what we do on this course.
...
*Note that this bit is optional.* You may have noted that getting the
...
Furthermore (or alternatively), now that we are growing the size
of the hash table dynamically to keep buckets small, does it make
sense to keep entries in a bucket ordered? How we do find out? (By
measuring of course -- but on what data set?) If we could retire
some complexity in the insert function (or any function), we have
simplified maintenance of this code which is great for the future.

#+begin_note
This step should give you what you need to do the two first
[[../achievements.org::O42][profiling]] and [[../achievements.org::O43][optimisation]] achievements.
...
7. Run your tests through Valgrind. Preferably using a [[../makefiles.org][Makefile]].
8. Make sure that the code passes your tests.
9. [[../github.org][Check your code and the profiling results into the appropriate
...
#+BEGIN_EXPORT html
<iframe src="https://giphy.com/embed/oBPOP48aQpIxq" width="480" height="308" frameBorder="0" class="giphy-embed" allowFullScreen></iframe><p><a href="https://giphy.com/gifs/thumbs-up-oBPOP48aQpIxq">via GIPHY</a></p>
#+END_EXPORT
...
- [[../misc/hash_table.h][=hash_table.h=]]
- [[../misc/list.h][=linked_list.h=]]
- [[../misc/iterator.h][=iterator.h=]]
...
For example, in addition to the one's hinted above, you likely
grasp and have evidence for demonstrating [[../achievements.org::J26][Achievement J26]] and
[[../achievements.org::J27][Achievement J27]], possibly also [[../achievements.org::D9][Achievement D9]] -- as long as you've
gone above and beyond what's been handed out. You probably have
code related to [[../achievements.org::M36][Achievement M36]]. Some error handling options are
related to [[../achievements.org::M38][Achievement M38]]. Your unittests -- regardless of what
approach you followed -- should get you at least one of the [[../achievements.org::Q][Q achievements]].
If you used Emacs diligently, then you're likely good to demonstrate [[../achievements.org::T55][Achievement T55]] (if not more).
...
7. *Optional* Please take time to [[mailto:z100-feedback@wrigstad.com][feedback on the assignment]].
...
# - *DONE* Give a catalogue structure
...
# TODO:
# Add clarification that Cunit must be used
# add clarification that test must be written regardless of branch
# add a forward pointer to the hash function
# exemplify with file structure
# Rewrite list step to extract list from the hash table
# Give out tests for the iterator as a good example
# Give out more instructions for finishing word frequency test
# Hand out a tree map for their perusal and to compare performance
# Divide text into separate files
# Possibly kill TDD path
# Work out a way to use macros to create typed data structures
# Move hash functions up
# Work with weekly google form evaluations to track stress and progress
# Better tag instructions
# Better function type instructions
# Give them one more week
# Practice hash functions in the labs?
# Check against how to C in 2016
# Add macOS keybindings to init
# Add a guide for code formatting on the save hook
