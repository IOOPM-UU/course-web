...
#+begin_important
*Intended start*: Start of Week 44 \\
*Soft deadline*: End of Week 46 \\
*Hard deadline*: End of Week 48
#+end_important
...
\textit{expression}  & ::= & \textit{expression} \mathop{+} \textit{term}  & \text{Addition}\\
            &  |  & \textit{expression} \mathop{-} \textit{term}  & \text{Subtraction}\\
            &  |  & \textit{term} \\
\textit{term}        & ::= & \textit{term} \mathop{*} \textit{factor}  & \text{Multiplication}\\
            &  |  & \textit{term} \mathop{/} \textit{factor}  & \text{Division} \\
            &  |  & \textit{factor} \\
\textit{factor} & ::= & \textit{primary} & \text{You can unify factor and primary} \\
\textit{primary}     & ::= & \texttt{(}~ \textit{assignment} ~\texttt{)} \\
            &  |  & \textit{unary} \\
...
#+Begin_note
The rules for expressions and terms are "left-recursive". That is,
one of the possible forms for expressions starts with another expression --
similarly for terms.
A straightforward implementation of a recursive descent parser for this grammar
can get stuck in an infinite loop. Fortunately, it is easy to see that e.g.
the rule for expressions means that an expression consists of a sequence of one
or more terms separated by plus or minus signs. (See the syntax diagram
in figure 4 of section 4.3.)  With this insight it is
easy to write a recursive descent parser for this simple case of left
recursion. Of course, the computations you make -- and the ASTs that 
you build in the proper calculator code -- have to follow the actual structure 
of the BNF rules. But that, too is easy to accomplish. See how it is done in
the example below!
#+end_note
...
4. Prepare a demonstration of z103 to give at the next lab. In
   addition to z103, pick another 2-3 achievements to tick off,
