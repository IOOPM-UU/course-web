...
What does the following code print?[fn::The answer is 42 44 44]
#+BEGIN_SRC c
int x = 42;
printf("%d ", x++);
printf("%d ", ++x);
...
What does the following code print?[fn::The answer is 43 43 44]
#+BEGIN_SRC c
int x = 42;
printf("%d ", ++x);
printf("%d ", x++);
...
(Brain teaser and sort-of trick question) What does the following
code print? [fn::Answer: Compiling this program generates a warning
saying that this operation may be underfined. On the computer
where I am writing this, using gcc (Ubuntu 6.3.0-12ubuntu2) 6.3.0
20170406 on an X86 64 running Ubuntu 17.04, the result is =85=.]
#+BEGIN_SRC c
int x = 42;
printf("%d.", x+++x);
#+END_SRC
...
copying function, that copies a string from ~b~ to ~a~. Do you
understand how it works?[fn::First, ~x = y~ returns the value of ~y~. Second, ~'\0'~ has the value ~0~ meaning it is the same as ~false~.]
...
do it on a single line, without breaking the FORMATTING
CONVENTIONS?
